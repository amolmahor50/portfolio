...
const Headline = ({
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            <div
                className={`relative space-y-4 ${textAlign}`}
            >
                {/* Optional badge */}
                {badge && (
                    <div className={`inline-flex tracking-tight items-center gap-2 px-4 py-1.5 rounded-none bg-primary/10 border border-primary/20 text-primary mb-2 ${center ? "justify-center" : ""}`}>
                        {badge}
                    </div>
                )}
