  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled
          ? "bg-white/90 backdrop-blur-xl py-3 border-b border-gray-200/80 shadow-xs"
          : "bg-white/60 backdrop-blur-md py-4 border-b border-gray-200/50"
      }`}
    >
        ...
        <div className="hidden lg:flex items-center space-x-6 lg:space-x-8">
          {navLinks.map((link) => (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
                `flex items-center space-x-2 text-base transition-all duration-300 hover:text-primary ${
          ...
          <Link to="/contact" className="btn-primary px-5 py-2.5 text-sm rounded-none">
            Contact Us
          </Link>
        </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden fixed inset-x-0 top-17.5 bg-background backdrop-blur-xl border-b border-border transition-all duration-500 overflow-hidden ${
        ...
        <div className="flex flex-col p-6 space-y-4">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center justify-between border-b border-gray-100 pb-3 text-base transition-all duration-300 ${
